...
app.get('/api/schools', async (req, res) => {
    try {
        const school = await SchoolModel.find({ id: 'school_002' });
        console.log(school)
        if (!school) {
...
app.get('/api/bookings', async (req, res) => {
    try {
        // Fetch all schools
        const schools = await SchoolModel.find({ id: 'school_002' });
...
app.put('/api/schools/:schoolId/teachers/:teacherId/dateses', async (req, res) => {
    try {
        ...
        const workTimes = req.body.workTime.map(wt => {
            const adjustedTime = new Date(new Date(wt.time).getTime() + 3 * 60 * 60 * 1000); // Adjust for your timezone
...
app.put('/api/schools/:schoolId/teachers/:teacherId/dates', async (req, res) => {
    try {
        ...
        const lang = teacher.data.lang.find(l => l.lang === req.body.lang);
        if (!lang) return res.status(404).json({ message: 'Language not found' });
        lang.level.find(lv => console.log(lv.levelName, req.body.levelName.levelName))
...
//         booking.workTime = req.body.workTime.map(wt => {
//             const adjustedTime = new Date(new Date(wt.time).getTime() + 3 * 60 * 60 * 1000);
//             return { ...wt, time: adjustedTime };
//         }).sort((a, b) => new Date(a.time) - new Date(b.time));

//         // Recalculate nonWorkTime
//         const nonWorkTimes = [];
//         const workTimes = booking.workTime;

//         // Start of the day non-working time
//         const startOfDay = new Date(new Date(workTimes[0].time).setUTCHours(0, 0, 0, 0));
//         const firstWorkTime = new Date(workTimes[0].time);
...
//             if (workTimes[i].nr === 0) {
//                 const currentStart = new Date(workTimes[i].time);
//                 const previousEnd = i > 0 ? new Date(workTimes[i - 1].time) : new Date(currentStart.setUTCHours(0, 0, 0, 0));
//                 const nextStart = (i < workTimes.length - 1) ? new Date(workTimes[i + 1].time) : new Date(currentStart.setUTCHours(23, 59, 0, 0));
...
//         booking.nonWorkTime = nonWorkTimes;
    ...
    try {
        ...
        for (const t of parsedTimes) {
            ...
            const school = await SchoolModel.findOne({
                "ESL.teacher.data.teacherId": teacherId, // Include teacherId in the query
            ...
            if (school) {
                // Find the correct teacher that matches teacherId, lang, and levelName
                const teacher = school.ESL.teacher.find(teacher =>
                    teacher.data.teacherId === teacherId && // Check for teacherId
                        ...
                        langItem.level.some(levelItem =>
                            ...
                            levelItem.date.some(date =>
                                date.workTime.some(workTime =>
                                    workTime.time.getTime() === new Date(parsedDate).getTime()
                                )
                            )
                        )
                    )
                ...
                if (!teacher) {
                    console.log(`No teacher found with work time ${parsedDate}, lang ${lang}, and level ${levelName}`);
                    continue;
                }
